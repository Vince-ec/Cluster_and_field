...
Gs = Gen_spec('GND', 21156, 1.25378, g102_lims=[7900, 11500], g141_lims=[11100, 16500],
                mdl_err = False, instr_err = False, phot_errterm = 0.03, decontam = True)  
...
def loglikelihood(X):
    ...
    sp.params['dust2'] = d
    sp.set_tabular_sfh(np.array([0.1, 0.5, 1, 2, 4, 7,10]),np.array([t1, t2, t3, t4, t5, t6, t7]))
...
############
####run#####
dsampler = dynesty.DynamicNestedSampler(loglikelihood, prior_transform, ndim = 11, sample = 'rwalk') 
dsampler.run_nested(wt_kwargs={'pfrac': 1.0}, dlogz_init=0.01, print_progress=False)

dres = dsampler.results
############
####save####
np.save(out_path + 'GND_21156_testfit_EF_tab.npy', dres) 
