...
############
####run#####
t_dsampler = dynesty.DynamicNestedSampler(delay_L, delay_prior, ndim = 6, sample = 'rwalk', bound = 'single',
                                  queue_size = 8, pool = Pool(processes=8)) 
t_dsampler.run_nested(wt_kwargs={'pfrac': 1.0}, dlogz_init=0.01, print_progress=False)

dres = t_dsampler.results
############
####save####
np.save(out_path + 'sim_test_tab_to_delay_single_{0}'.format(runnum), dres) 
...
np.save(out_path + 'sim_test_tab_to_delay_single_{0}_lwa'.format(runnum), lwa) 
