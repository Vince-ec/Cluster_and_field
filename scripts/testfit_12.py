...
sim1.Make_sim(wave2, flux2 * 10**11* lsol_to_fsol / (4 * np.pi * (D_l*conv)**2), specz, rndstate = rndseed)
...
############
####save####
np.save(out_path + 'sim_test_delay_to_delay_multi_{0}'.format(runnum), dres) 
...
np.save(out_path + 'sim_test_delay_to_delay_multi_{0}_lwa'.format(runnum), lwa) 

params = ['m', 'a','t', 'z', 'd', 'lm']
for i in range(len(params)):
    t,pt = Get_posterior(dres,i)
    np.save(pos_path + 'sim_test_delay_to_delay_multi_{0}_P{1}'.format(runnum, params[i]),[t,pt])

bfm, bfa, bft, bfz, bfd, bflm = dres.samples[-1]

np.save(pos_path + 'sim_test_delay_to_delay_multi_{0}_bfit'.format(runnum),
        [bfm, bfa, bft, bfz, bfd, bflm, dres.logl[-1]])
    
dres.samples[:,1] = lwa
m,Pm = Get_posterior(dres, 1)
np.save(pos_path + 'sim_test_delay_to_delay_multi_{0}_Plwa'.format(runnum),[m,Pm])
