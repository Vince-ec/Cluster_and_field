...
def Galfit_prior(u):
    ...
    m1, m2, m3, m4, m5, m6, m7, m8, m9, m10 = logsfr_ratios_to_masses(logmass = 0, logsfr_ratios = taus, agebins = agebins)
    
    lm = Gaussian_prior(u[11], [9.5, 12.5], 11, 0.75)
  
    z = stats.norm.ppf(u[12],loc = specz, scale = zscale)
    
    d = log_10_prior(u[13],[1E-3,2])
    
    bp1 = Gaussian_prior(u[14], [-0.1,0.1], 0, 0.05)
    rp1 = Gaussian_prior(u[15], [-0.05,0.05], 0, 0.025)
    ...
    lwa = get_lwa([m, a, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10], agebins,sp)[0]
    
    return [m, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, lm, z, d, bp1, rp1, ba, bb, bl, ra, rb, rl, lwa]

def Galfit_L(X):
    m, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, lm, z, d, bp1, rp1, ba, bb, bl, ra, rb, rl, lwa = X
...
np.save(out_path + '{0}_{1}_tabfit_test'.format(field, galaxy), dres) 

##save out P(z) and bestfit##
params = ['m', 'm1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8', 'm9', 'm10', 'lm',
          'z', 'd', 'bp1', 'rp1', 'ba', 'bb', 'bl', 'ra', 'rb', 'rl', 'lwa']
for i in range(len(params)):
    t,pt = Get_posterior(dres,i)
    np.save(pos_path + '{0}_{1}_tabfit_test_P{2}'.format(field, galaxy, params[i]),[t,pt])

bfm, bfm1, bfm2, bfm3, bfm4, bfm5, bfm6, bfm7, bfm8, bfm9, bfm10, bflm, bfz, bfd,\
    bfbp1, bfrp1, bfba, bfbb, bfbl, bfra, bfrb, bfrl, bflwa= dres.samples[-1]

np.save(pos_path + '{0}_{1}_tabfit_test_bfit'.format(field, galaxy),
        [bfm, bfm1, bfm2, bfm3, bfm4, bfm5, bfm6, bfm7, bfm8, bfm9, bfm10, bflm, bfz, bfd,
