...
def Galfit_prior(u):
    ...
    a = Gaussian_prior(u[1], [agelim/2, agelim], agelim, 0.5)
...
np.save(out_path + '{0}_{1}_tabMfit9'.format(field, galaxy), dres) 

params = ['m', 'a', 'm1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8', 'm9', 'm10','z', 'd']
for i in range(len(params)):
    t,pt = Get_posterior(dres,i)
    np.save(pos_path + '{0}_{1}_tabMfit9_P{2}'.format(field, galaxy, params[i]),[t,pt])

bfm, bfa, bfm1, bfm2, bfm3, bfm4, bfm5, bfm6, bfm7, bfm8, bfm9, bfm10, bfz, bfd= dres.samples[-1]

np.save(pos_path + '{0}_{1}_tabMfit9_bfit'.format(field, galaxy),
