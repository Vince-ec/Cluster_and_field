...
def Galfit_prior(u):
    ...
    z = stats.norm.ppf(u[3],loc = specz, scale = 0.005)
    
    d = (1.5)*u[4] + 0.01
    
    bp1 = Gaussian_prior(u[5], [-0.5,0.5], 0, 0.25)
    
    rp1 = Gaussian_prior(u[6], [-0.5,0.5], 0, 0.25)
    ...
    lwa = get_lwa_delay([m, a, t], get_agebins(a), sp)

    return [m, a, t, z, d, bp1, rp1, ba, bb, bl, ra, rb, rl, lwa]

def Galfit_L(X):
    m, a, t, z, d, bp1, rp1, ba, bb, bl, ra, rb, rl, lwa = X
...
params = ['m', 'a','t', 'z', 'd','bp1', 'rp1', 'ba', 'bb', 'bl', 'ra', 'rb', 'rl', 'lwa']

for i in range(len(params)):
    t,pt = Get_posterior(dres,i)
    np.save(pos_path + '{0}_{1}_delayfit_P{2}'.format(field, galaxy, params[i]),[t,pt])

bfm, bfa, bft, bfz, bfd, bfbp1, bfrp1, bfba, bfbb, bfbl, bfra, bfrb, bfrl, bflwa= dres.samples[-1]

np.save(pos_path + '{0}_{1}_delayfit_bfit'.format(field, galaxy),
        [bfm, bfa, bft, bfz, bfd, bfbp1, bfrp1, bfba, bfbb, bfbl, bfra, bfrb, bfrl, bflwa, dres.logl[-1]])
