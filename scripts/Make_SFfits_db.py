...
for p in range(len(params)):
    ...
    for i in range(len(select.index)):
        try:
            x,px = np.load('../Casey_data/posteriors/{0}_{1}_SFfit_p1_P{2}.npy'.format(select.field[select.index[i]], select.id[select.index[i]], params[p]))
...
tabfits.to_pickle('../Casey_data/SF_db_p1.pkl')